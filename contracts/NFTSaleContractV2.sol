    ...
    ERC1155Holder
{
    ...
    bytes32 public constant RESELLER_ROLE = keccak256("RESELLER_ROLE"); // for buy nft by reseller
    ...
    // buy nft by reseller. is same as buy nft but have nftTo and refCode
    function buyNftByReseller(
        uint256 nftId,
        uint256 nftQuantity,
        address nftTo,
        string memory refCode
    ) external onlyRole(RESELLER_ROLE) whenNotPaused nonReentrant {
        // require nft quantity is not zero
        require(nftQuantity != 0, "NFTSaleContract: nft quantity is zero");
        ...
        // increase sales share amount of platform
        platformSalesShareAmountTotal += platformShareAmount;
        partnerSalesShareAmountTotal += partnerShareAmount;
